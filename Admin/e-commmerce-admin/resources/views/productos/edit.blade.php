...
@section('title', 'Editar Producto - Global AutoParts')
            ...
            <div class="bg-white rounded-2xl shadow-xl p-6 sm:p-8 border border-blue-100">
                <div class="flex items-center gap-3 mb-6">
                    ...
                    <h1 class="text-2xl font-bold text-blue-900">
                        Editar Producto: {{ $productoArray['nombre'] ?? 'N/A' }}
                    </h1>
                </div>
                ...
                <form action="{{ route('productos.update', $productoArray['id']) }}" method="POST"
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <div class="bg-blue-50 rounded-xl p-6 border border-blue-200">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" name="nombre" id="nombre"
                                    value="{{ old('nombre', $productoArray['nombre'] ?? '') }}" required
                            ...
                            <div>
                                ...
                                <textarea name="descripcion" id="descripcion" rows="4"
                                    ...
                                    placeholder="Describa las características del producto...">{{ old('descripcion', $productoArray['descripcion'] ?? '') }}</textarea>
                                @error('descripcion')
                                    <span class="text-sm text-red-600 mt-1 block">{{ $message }}</span>
                                @enderror
                            </div>
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="number" name="precio" id="precio"
                                            value="{{ old('precio', $productoArray['precio'] ?? 0) }}" required
                                            step="0.01" min="0"
                            ...
                            <div
                                ...
                                <div>
                                    ...
                                    <input type="number" name="stock" id="stock"
                                        value="{{ old('stock', $productoArray['stock'] ?? 0) }}" required min="0"
                                        ...
                                        @foreach ($categorias as $categoria)
                                            @php
                                                $categoriaArray = is_array($categoria)
                                                    ...
                                                    : $categoria->toArray();
                                                $selected =
                                                    old('categoria_id', $productoArray['categoria_id'] ?? '') ==
                                                    $categoriaArray['id'];
                                            @endphp
                                    ...
                                    <select name="marca_id" id="marca_id" required
                                        ...
                                        @foreach ($marcas as $marca)
                                            @php
                                                ...
                                                $selected =
                                                    old('marca_id', $productoArray['marca_id'] ?? '') ==
                                                    $marcaArray['id'];
                                            @endphp
                                            <option value="{{ $marcaArray['id'] }}" {{ $selected ? 'selected' : '' }}>
                        ...
                        <div class="bg-gradient-to-br from-cyan-50 to-blue-50 rounded-xl p-6 border border-cyan-200">
                            ...
                            <p class="text-sm text-cyan-700 mb-6">
                                Actualice las imágenes del producto. Deje en blanco para conservar las imágenes actuales.
                            </p>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                @for ($i = 1; $i <= 3; $i++)
                                    @php
                                        $fieldName = "imagen_$i";
                                        $imageUrl = $productoArray[$fieldName] ?? null;
                                        $isRequired = $i === 1;
                                    @endphp
                                        ...
                                        <div class="mb-3">
                                            @if ($imageUrl)
                                                <img src="{{ asset('storage/' . $imageUrl) }}"
                                                    ...
                                                    onerror="this.src='https://via.placeholder.com/150?text=Imagen+{{ $i }}+No+Disponible'">
                                            @else
                                                <div
                                                    class="mx-auto h-32 w-32 bg-cyan-100 border-2 border-cyan-300 rounded-xl flex items-center justify-center shadow-lg">
                                                    <svg class="w-12 h-12 text-cyan-400" fill="none"
